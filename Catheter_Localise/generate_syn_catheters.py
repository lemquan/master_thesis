...
def gen_synthetic_data():
    ...
    for i in xrange(5000):
        ...
        # plt.show()
    print 'done creatign fake wires set'
    with h5py.File('fake_wire_points_big.h5', 'w') as hf:
        hf.create_dataset('syn_data_x', data=syn_data_x)
        hf.create_dataset('syn_data_y', data=syn_data_y)

def blend_to_background(path):

    fake_wires = np.zeros((5000, 512*512))
    masks = np.zeros((5000, 512*512))
    with h5py.File(path,'r') as hf:
        # Print a list of arrays in this file
        keys = hf.keys()

        # this is the synthetic generated coordinates of a guide wire (x,y)
        syn_data_x = np.array(hf.get(keys[0]))
        syn_data_y = np.array(hf.get(keys[1]))

        for i in xrange(len(syn_data_x)): #len(syn_data_x)
            # if i < 1000:
            #     bg = cv2.imread('background_1.png', 0) #load the background in greyscale
            # elif 1000 < i > 2000:
            #     bg = cv2.imread('background_2.png', 0) #load the background in greyscale
            # else:
            bg = cv2.imread('background_3.png', 0)
            ...
            fin_image, gt_mask = interpolate_syn_data(scaled_syn_x, scaled_syn_y, bg)
            fn = 'fake_wires/' + 'fake_wire_'+ str(i) +'.png'
            fn_mask = 'fake_wires/' + 'fake_wire_mask_'+ str(i) +'.png'
            plt.imsave(fn, fin_image, cmap=cm.Greys_r)
            plt.imsave(fn_mask, gt_mask, cmap=cm.Greys_r)

            # save the image and mask into an ndarry
            fake_wires[i] = fin_image.flatten()
            masks[i] = gt_mask.flatten()
    # with h5py.File('syn_data_mask_no_bg.h5', 'w') as hf:
    #     hf.create_dataset('img', data=fake_wires)
    #     hf.create_dataset('mask', data=masks)
...
def interpolate_syn_data(x,y, bg):
    ...
    width = 10 # set to 2 originally
    for i in xrange(len(x)):
        x_coord = x[i].astype(np.int32)
        y_coord = y[i].astype(np.int32)
        canvas[y_coord, x_coord-width:x_coord+width+1] = 99

    # rotate the guide wire
    rows, cols = bg.shape
    M = cv2.getRotationMatrix2D((cols/2, rows/2), 180, 1)
    rot = cv2.warpAffine(canvas, M, (cols, rows))

    # get the new coordinates to create a binary mask for ground truth
    gt_x,gt_y = np.where(rot == 99)
    mask = np.zeros((512,512)) # create the black mask, 0 = black
    for i in xrange(len(gt_x)):
        mask[gt_x[i], gt_y[i]] = 255 # white

    # apply a Gaussian blur to smooth the wire
    blur = cv2.GaussianBlur(rot,(3,9), 0)

    # darken the wire by changing contrast
    darken = contrast_wire(blur, 1, 1)

    # blend two images
    #blend = cv2.addWeighted(bg,1,darken, 0.2, 1) # commented out if just generating syn with no background
...
def main():
    ...
    #gen_synthetic_data() # run this first to create fake points
    #blend_to_background('fake_wire_points.h5')
    blend_to_background('fake_wire_points_big.h5')
